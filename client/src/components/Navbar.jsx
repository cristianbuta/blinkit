        ...
        <nav className='w-full flex md:justify-center justify-between items-center p-4'>
            ...
            <ul className='text-white md:flex hidden list-none flex-row justify-between items-center'>
                {navItems.map((title, index) => {
                    return <NavbarItem key={title + index} title={title} />
                })}
            ...
            <div className='flex relative'>
                {toggleMenu
                    ? <AiOutlineClose fontSize={28} className='text-white md:hidden cursor-pointer' onClick={() => { setToggleMenu(!toggleMenu) }} />
                ...
                {toggleMenu && (
                    <ul className='z-10 fixed top-0 -right-2 p-3 w-[70vw] h-screen shadow-2xl md:hidden list-none flex flex-col justify-start items-end rounded-md blue-glassmorphism text-white animate-slide-in'>
                        ...
                        {navItems.map((title, index) => {
                            return <NavbarItem key={title + index} title={title} classProps='my-2 text-lg'/>
                        })}
                    </ul>
                )}
            </div>
